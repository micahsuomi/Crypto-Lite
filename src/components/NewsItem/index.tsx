...
const NewsItem = ({
  ...
  return (
    <div className="news-item" style={{ backgroundColor: theme.backgroundColor, color: theme.text}}>
      ...
      <div className="news-item__right">
        <a href={`${articleUrl}`} target="blank">
          <h3 className="news-header" style={{ color: theme.text }}>{title}</h3>
        </a>
        <p className="news-item-paragraph news-item__hide-mobile">
          {description}
        </p>
        <p className="categories" style={{ color: theme.text}}>{categories}</p>
